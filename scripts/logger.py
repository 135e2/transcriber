...
class RedirectStreamToLogger:
    ...
    def write(self, buf):
        for line in buf.rstrip().splitlines():
            # Here we redirect this certain string to logging.INFO, since there's no actual error written to stderr in the `translators` library:
            # https://github.com/UlionTse/translators/blob/ec2db28763c7cb02bec2074766680f3f85f6ec7a/translators/server.py#L244
            if re.search("Using state \S* server backend.", line):
